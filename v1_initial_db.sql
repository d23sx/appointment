...
INSERT INTO site_info (site_id, site_name, description, start_time, end_time, ticket_start_number, is_non_bahraini, is_active) VALUES
('ISA_TOWN', 'Isa Town', 'Main site_info', '08:00:00', '14:00:00', 5000, FALSE, TRUE),
...
INSERT INTO site_info_time_slot (site_id, slot_id, start_time, end_time, max_limit, available_days, is_active) VALUES
('ISA_TOWN', 1, '08:00:00', '09:00:00', 10, ARRAY[0,1,2,3,4], TRUE),
...
INSERT INTO site_info_time_slot (site_id, slot_id, start_time, end_time, max_limit, available_days, is_active) VALUES
('ISA_TOWN', 7, '08:00:00', '09:00:00', 10, ARRAY[0,1,2,3,4,5,6], TRUE),
('ISA_TOWN', 8, '09:00:00', '10:00:00', 10, ARRAY[0,1,2,3,4,5,6], TRUE);

INSERT INTO site_ticket_allocation (site_id, allocation_date, start_number, next_available_number, is_active) VALUES
('ISA_TOWN', '2025-11-27', 5000, 5000, TRUE),
...
CREATE OR REPLACE FUNCTION is_slot_available_on_day(
    p_slot_id INT,
    p_site_id VARCHAR,
    p_date DATE
)
...
BEGIN
    v_day_of_week := EXTRACT(DOW FROM p_date)::INT;  -- 0=Sunday, 1=Monday, ..., 6=Saturday
    
    SELECT available_days INTO v_available_days
    FROM site_info_time_slot
    WHERE id = p_slot_id AND site_id = p_site_id;
...
CREATE OR REPLACE FUNCTION get_day_name(p_day_of_week INT)
RETURNS VARCHAR AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION is_customer_late(
...
DECLARE
    ...
    v_minutes_late INT;
BEGIN
    SELECT a.appointment_time, a.arrival_time INTO v_appointment_time, v_arrival_time
    FROM appointment a
    WHERE a.id = p_appointment_id;
    
    IF v_arrival_time IS NULL THEN
        RETURN FALSE;
    END IF;
    
    v_minutes_late := EXTRACT(EPOCH FROM (v_arrival_time - 
...
CREATE OR REPLACE FUNCTION get_waiting_time_minutes(
    p_appointment_id INT
)
RETURNS INT AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION get_site_info_avg_waiting_time(
    p_site_id VARCHAR,
    p_date DATE
)
RETURNS TABLE (
...
CREATE OR REPLACE FUNCTION is_customer_blocked(
...
CREATE OR REPLACE FUNCTION mark_appointment_not_attended(
    p_appointment_id INT,
    p_cpr_or_passport VARCHAR,
    p_marked_by INT,
...
CREATE OR REPLACE FUNCTION allocate_next_ticket(
    p_site_id VARCHAR,
...
DECLARE
    v_next_number INT;
BEGIN
    SELECT next_available_number INTO v_next_number
